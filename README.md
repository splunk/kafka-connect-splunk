...
7. Run the following command to create a connector tasks. Adjust the `topics`, `tasks.max`, `indexes`, `sources`, `sourcetypes` and `hec` etc settings if necessary
    ...
    "config": {
       ...
       "splunk.hec.json.event.enrichment": "<key value pairs separated by comma, only applicable /event HEC>",
       ...
       "splunk.hec.max.http.connection.per.channel": "<number of http connections>",
       "splunk.hec.total.channels": "<number of channels>",
       "splunk.hec.max.batch.size": "<max number of kafka records post in batch>",
